...
export const registerWorkLogTools = (server) => {
    server.tool("get_issue_worklogs", "Get all worklogs for a specific issue", {
        ...
        const response = await makePlaneRequest("GET", `workspaces/${process.env.PLANE_WORKSPACE_SLUG}/projects/${project_id}/issues/${issue_id}/worklogs/`);
        return {
            content: [
                {
                    type: "text",
    ...
    server.tool("get_total_worklogs", "Get total logged time for a project", {
        project_id: z.string().describe("The uuid identifier of the project to get total worklogs for"),
    }, async ({ project_id }) => {
        const response = await makePlaneRequest("GET", `workspaces/${process.env.PLANE_WORKSPACE_SLUG}/projects/${project_id}/total-worklogs/`);
        return {
            content: [
                {
                    type: "text",
    ...
    server.tool("create_worklog", "Create a new worklog for an issue", {
        project_id: z.string().describe("The uuid identifier of the project containing the issue"),
        ...
        const response = await makePlaneRequest("POST", `workspaces/${process.env.PLANE_WORKSPACE_SLUG}/projects/${project_id}/issues/${issue_id}/worklogs/`, worklog_data);
        return {
            content: [
                {
                    type: "text",
    ...
    server.tool("update_worklog", "Update an existing worklog", {
        project_id: z.string().describe("The uuid identifier of the project containing the issue"),
    ...
    server.tool("delete_worklog", "Delete a worklog", {
        ...
        await makePlaneRequest("DELETE", `workspaces/${process.env.PLANE_WORKSPACE_SLUG}/projects/${project_id}/issues/${issue_id}/worklogs/${worklog_id}/`);
        return {
            content: [
                {
                    type: "text",
